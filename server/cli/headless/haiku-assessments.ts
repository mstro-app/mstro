...
export async function assessPrematureCompletion(
  ...
  const prompt = [
    ...
    '',
    'IMPORTANT: The text below is the final portion of the response. Earlier progress',
    'messages like "Let me check...", "Let me try..." were followed by tool calls that',
    'executed successfully — those are COMPLETED actions, not pending work.',
    'Focus on the LAST sentence or paragraph to judge completeness.',
    '',
    `Final response text (last ${ctx.responseTail.length} chars):`,
    ctx.responseTail,
    '',
    'INCOMPLETE signals: announcing next steps with NO tool call after ("Now I\'ll...",',
    '"Next I need to...", "I\'ll continue with..."), describing work that will happen',
    'next as the very last thing said.',
    '',
    'COMPLETE signals: summarizing what was done, confirming changes, reporting results,',
    'asking the user a question, past-tense descriptions, "all done", "changes applied",',
    'referencing finished state. A short summary after many tool calls is COMPLETE.',
    '',
    'Respond in EXACTLY this format (2 lines, no extra text):',
