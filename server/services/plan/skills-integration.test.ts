 ...
 * agent-loader.ts. All skills are baked into the agents/ directory and always available.
 *
...
const SYSTEM_SKILLS = [
  ...
  'commit-message', 'pr-description', 'code-review', 'fix-quality',
] as const;
...
describe('expected {{variable}} placeholders in baked-in skills', () => {
...
describe('backwards compatibility — baked-in skill interpolation', () => {
...
describe('system skills resolve from arbitrary working directories', () => {
  it('finds system agent even when workingDir has no project skills', () => {
