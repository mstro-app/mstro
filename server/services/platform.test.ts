...
// Compute expected credentials filename the same way platform-credentials.ts does
const expectedCredentialsFile = (() => {
  const env = process.env.MSTRO_ENV || 'production'
  if (env === 'staging') return '/home/testuser/.mstro/credentials-staging.json'
  if (env === 'dev') return '/home/testuser/.mstro/credentials-dev.json'
  return '/home/testuser/.mstro/credentials.json'
})()
  ...
  describe('Connection Lifecycle', () => {
    describe('connect()', () => {
      it('should read credentials from disk', () => {
        ...
        expect(mockFs.existsSync).toHaveBeenCalledWith(expectedCredentialsFile)
        expect(mockFs.readFileSync).toHaveBeenCalledWith(
          expectedCredentialsFile,
          'utf-8'
        )
      })
    ...
    describe('reconnection behavior', () => {
      ...
      it('should include protocolVersion=1 in connection params', () => {
        const credentials = {
          token: 'test-token',
          userId: 'user-123',
          email: 'test@example.com',
        ...
        const wsUrl = WebSocketConstructor.mock.calls[0][0]
        // Must match minProtocolVersion in server/src/relay/version-policy.ts.
        // If the server's floor moves past 1, the CLI must bump in lockstep.
        expect(wsUrl).toContain('protocolVersion=1')
      })

      it('should print upgrade message and stop hammering the server on protocol-too-old close', () => {
        const credentials = {
          token: 'test-token',
          userId: 'user-123',
          email: 'test@example.com',
        ...
        // Server closes with 1008 + reason 'protocol-too-old'. (HTTP 426
        // before upgrade follows the same code path via the
        // `unexpected-response` listener; covered separately below.)
        lastWebSocketInstance!.triggerClose(1008, 'protocol-too-old')

        expect(console.error).toHaveBeenCalledWith(
          expect.stringContaining('Mstro CLI is out of date for this orchestra')
        )
        expect(console.error).toHaveBeenCalledWith(
          expect.stringContaining('Run `mstro` again to upgrade')
        )
        expect(onError).toHaveBeenCalledWith(
          expect.stringContaining('Mstro CLI is out of date')
        )

        // Tight reconnect loop must NOT fire — neither at the regular
        // exponential-backoff cadence (~1s, 2s, 4s, ... up to 30s) nor
        // anywhere inside the sentinel window. The user is expected to
        // re-run `mstro`, which spawns a fresh process; this stale one
        // must stay quiet.
      ...
      it('should treat HTTP 426 unexpected-response as protocol-too-old', () => {
        const credentials = {
          token: 'test-token',
          userId: 'user-123',
          email: 'test@example.com',
        ...
        // The `ws` library emits `unexpected-response` BEFORE the close —
        // simulate that path. The MockWebSocket extends EventEmitter, so
        // platform.ts's `.on('unexpected-response', ...)` listener attaches
        // to it the same way it would on a real `ws` instance.
        lastWebSocketInstance!.emit('unexpected-response', {}, { statusCode: 426 })

        // After 426, the underlying socket closes with 1006 (the `ws`
        // library doesn't synthesize a protocol-aware close after a failed
        // upgrade).
        lastWebSocketInstance!.triggerClose(1006, '')
