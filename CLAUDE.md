...
- The CLI runs on the user's local machine. Destructive file ops, process spawning, and port binding are real. For filesystem writes, process spawning, or port binding: confirm the path/port is correct and the operation is reversible before running.
- WebSocket messages flow: web -> server -> cli. If you change a message handler in cli, update the corresponding sender in server/src/relay/ and web/src/services/.
